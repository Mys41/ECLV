# 该脚本用于Transformer编码器和解码器的带有相对位置嵌入的自注意力
# 它同时支持标记级别和句子级别的相对位置
# 我们的实现参考T5
# 更多细节请参阅<https://github.com/google-research/text-to-text-transfer-transformer>
# TODO:在发布的版本中，我们将合并一些模块以减少这部分代码
import math
import warnings
import numpy as np
import torch
...
# 计算相对位置的桶编号
# relative_position表示相对位置。它是一个张量，其中每个元素表示两个位置之间的相对距离
# num_buckets参数表示桶的数量。它是一个整数，表示将相对位置划分为多少个桶
# max_distance参数表示最大距离。它是一个整数，表示相对位置的最大值
# bidirectional参数表示是否为双向。如果为真，则表示相对位置可以为负数；否则，相对位置只能为非负数。
def _relative_position_bucket(relative_position, num_buckets, max_distance, bidirectional):
    n = -relative_position
    ret = 0
    # 如果bidirectional为真，则表示相对位置可以为负数。在这种情况下，代码会将num_buckets除以2，以便为正数和负数分别分配一半的桶
    if bidirectional:
...
class RelativePositionBias(nn.Module):
    def __init__(self, embed_dim, n_heads, num_buckets, max_distance, bidirectional):
        ...
        if self.num_buckets % 4 != 0 or self.max_distance % self.num_buckets != 0:
            # num_buckets建议是4的倍数，max_distance建议是num_buckets的倍数
...
class NgramMultiheadAttention(nn.Module):
    ...
    def forward(self, query,
        ...
        # below code slower than above for loop k_ngram = torch.cat([k_main.unsqueeze(0).repeat(self.ngram, 1, 1,
        # 1) , torch.cat(k_predict_list).view(self.ngram, -1, real_tgt_len, self.head_dim)], 2)
        ...
        # below code slower than above for loop v_ngram = torch.cat([v_main.unsqueeze(0).repeat(self.ngram, 1, 1, 1)
        # , torch.cat(v_predict_list).view(self.ngram, -1, real_tgt_len, self.head_dim)], 2)
