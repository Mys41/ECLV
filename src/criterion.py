...
class VEDLoss(FairseqCriterion):
    def __init__(self, args, task):
        ...
        self.strategy_rc_loss_weight = args.strategy_rc_loss_weight
    ...
    def forward(self, model, sample, reduce=True):
        ...
        # 策略的重构损失
        strategy_logits = logits[:, 0, :].unsqueeze(1)  # 获取生成回答的首个标记(即策略标记)的logits
        strategy_targets = targets[:, :, 0].unsqueeze(2)  # 获取真实回答的首个标记
        strategy_lprobs = F.log_softmax(strategy_logits.view(-1, strategy_logits.size(-1)), dim=-1, dtype=torch.float32)
        strategy_rc_loss = F.nll_loss(
            input=strategy_lprobs,
            target=strategy_targets.view(-1),
            reduction='sum', ignore_index=self.padding_idx)
        ...
        if self.eps > 0.:
            ...
            strategy_rc_loss = strategy_rc_loss * 300
        ...
        loss = rc_loss
        if strategy_rc_loss is not None:
            loss = loss + self.strategy_rc_loss_weight * strategy_rc_loss
        ...
        logging_output = {
            ...
            'strategy_rc_loss': utils.item(strategy_rc_loss.data) if reduce else strategy_rc_loss.data,
        }
    ...
    def aggregate_logging_outputs(logging_outputs):
        ...
        strategy_rc_loss = sum(log.get('strategy_rc_loss', 0) for log in logging_outputs)
        ...
        agg_output = {
            ...
            'strategy_rc_loss': strategy_rc_loss / sample_size / math.log(2),
